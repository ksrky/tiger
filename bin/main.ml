module Translate = Tiger.Translate
module Frame = Tiger.Frame
module Parse = Tiger.Parse
module FindEscape = Tiger.FindEscape
module Semant = Tiger.Semant
module Canon = Tiger.Canon
module Assem = Tiger.Assem
module Codegen = Tiger.Codegen
module Temp = Tiger.Temp
(*module R = Tiger.RegAlloc*)

let emitproc (out : out_channel) : Frame.frag -> unit = function
  | Frame.PROC{body; frame} ->
    let () = print_endline ("emit " ^ Frame.name frame) in
    (* let _ = Printtree.printtree(out,body); *)
    let stms = Canon.linearize body in
    (* let _ = app (fn s => Printtree.printtree(out,s)) stms; *)
    let stms' = Canon.traceSchedule(Canon.basicBlocks stms) in
...
let withOpenFile (fname : string) (f : out_channel -> unit) : unit = 
    ...
    try let v = f out in close_out out; v
