...
let rec linearize (stm0 : T.stm) : T.stm list =
  let ( % ) x y =
    match (x, y) with T.EXP (T.CONST _), x -> x | x, T.EXP (T.CONST _) -> x | x, y -> T.SEQ (x, y)
  in
...
let enterblock b t =
  match (b, t) with (T.LABEL s :: _ as b), table -> Symbol.enter (table, s, b) | _, table -> table
...
let rec trace (table, (T.LABEL lab :: _ as b), rest) =
  ...
  | most, T.CJUMP (opr, x, y, t, f) -> (
    ...
    | Some (_ :: _ as b'), _ -> most @ [T.CJUMP (T.notRel opr, x, y, f, t)] @ trace (table, b', rest)
    | _ ->
