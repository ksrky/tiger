...
type enventry =
  | VarEntry of {access: Tr.access; ty: ty}
...
let base_venv =
  Symbol.init
    (List.map
       ...
       [ ("print", [T.STRING], T.UNIT); ("flush", [], T.UNIT); ("getchar", [], T.STRING)
       ; ("ord", [T.STRING], T.INT); ("chr", [T.INT], T.STRING); ("size", [T.STRING], T.INT)
       ; ("substring", [STRING; INT; INT], INT); ("concat", [STRING; STRING], STRING)
       ; ("not", [INT], INT); ("exit", [INT], UNIT) ] )
