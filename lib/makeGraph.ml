let instr2graph (ilist : Assem.instr list) : Flow.flowgraph * Flow.Graph.node list =
    ...
    match instr with
      | Assem.OPER{dst; src; jump; _} ->
        (match jump with
          ...
          | Some jmps -> List.iter (map_latter_labeled_edge node) jmps); (*temp*)
        Flow.FGRAPH { control
  ...
  let Flow.FGRAPH{control;_} as resultFGraph = List.fold_left makeGraph initialFGraph (List.rev ilist) (* note: reverse order *)
