...
let nodes (g : graph) =
  let rec f i = if isBogus g.(i) then [] else (g, i) :: f (i + 1) in
  f 0

let succ ((g, i) : node) : node list =
  let (NODE {succ= s; _}) = g.(i) in
  List.map (augment g) s

let pred ((g, i) : node) : node list =
  let (NODE {pred= p; _}) = g.(i) in
...
let newNode (g : graph) =
  let rec look (lo, hi) =
    (* i < lo indicates i in use i >= hi indicates i not in use *)
    if lo = hi then (Array.set g lo emptyNode; (g, lo))
    else
      let m = (lo + hi) / 2 in
      if isBogus g.(m) then look (lo, m) else look (m + 1, hi)
  in
  look (0, Array.length g)
