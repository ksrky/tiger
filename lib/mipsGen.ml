...
let codegen _ stm =
  ...
  and munchExp = function
    ...
    | T.MEM(T.CONST i) ->
      result (fun r -> emit (A.OPER { assem="\tlw\t`d0, " ^ string_of_int i ^ "(`s0)\n"
                                    ...
                                    ; jump=None}))
    | T.MEM(e1) ->
      result (fun r -> emit (A.OPER { assem="\tlw\t`d0, 0(`s0)\n" (* temp: or (`s1)*)
                                    ; src=[munchExp e1]
                                    ; dst=[r]
                                    ; jump=None}))
    (* binop *)
    | T.BINOP(T.PLUS, e1, T.CONST i) ->
      result (fun r -> emit (A.OPER { assem="\taddiu\t`d0, `s0, " ^ string_of_int i ^ "\n"
                                    ; src=[munchExp e1]
                                    ; dst=[r]
                                    ; jump=None}))
    ...
    | T.BINOP(T.MINUS, e1, e2) ->
      result (fun r -> emit (A.OPER { assem="\tsub\t`d0, `s0, `s1\n"
                                    ; src=[munchExp e1; munchExp e2]
                                    ; dst=[r]
                                    ; jump=None}))
    | T.BINOP(T.MUL, e1, e2) ->
      result (fun r -> emit (A.OPER { assem="\tmul\t`d0, `s0, `s1\n"
                                    ; src=[munchExp e1; munchExp e2]
                                    ; dst=[r]
                                    ; jump=None}))
    | T.BINOP(T.DIV, e1, e2) ->
      result (fun r -> emit (A.OPER { assem="\tdiv\t`d0, `s0, `s1\n"
                                    ; src=[munchExp e1; munchExp e2]
                                    ; dst=[r]
                                    ; jump=None}))
    ...
    (* name *)
    | T.NAME lab ->
      result (fun r -> emit (A.OPER { assem="\tla\t`d0, " ^ Symbol.name lab ^ "\n"
                                    ; src=[]
                                    ; dst=[r]
                                    ; jump=None}))
  ...
  and munchArgs = function
    ...
    | (i, arg::rest) ->
      ...
      munchStm(T.MOVE(T.TEMP dst, T.TEMP src)); (* temp: spill *)
