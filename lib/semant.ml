...
and actual_ty(ty, pos) =
  let rec walk = function
    | T.NAME(id, ref) -> (match !ref with
      | Some(ty) -> walk ty
      | None -> error pos ("unknown type. " ^ S.name id); T.NIL)
    | ty -> ty
  in walk ty
...
and transDecs(venv, tenv, level, breakpoint, decs) : (venv * tenv * Tr.exp list) =
  let transDec (venv, tenv, exps) = function
    | A.TypeDec tydecs ->
      ...
      List.iter (fun {A.name; A.ty; _} ->
        match S.look(tenv', name) with
          | Some(T.NAME(_, ref)) ->
            ref := Some(transTy(tenv', ty))
          | _ -> ErrorMsg.impossible "Semant.transDecs") tydecs;
    ...
    | A.FunctionDec fundecs ->
      ...
      let transparam{A.name; A.escape; A.typ; A.pos; _} = match S.look(tenv, typ) with
        ...
        | None -> error pos ("unknown type." ^ S.name typ); (name, true, T.NIL, pos) in
      let transfun_header(venv, {A.name; A.params; A.result; _}) =
        ...
        let formals = List.map (fun {A.typ; A.pos; _} -> match S.look(tenv, typ) with
          | Some(ty) -> ty
          | None -> error pos ("unknown type." ^ S.name typ); T.NIL) params in
      ...
      let transfun_body{A.name; A.params; A.result; A.body; _} =
        match S.look(venv', name) with
          | Some(E.FunEntry{level=newlevel;_}) ->
            ...
            let {exp=body_exp; ty=body_ty} = transExp(venv'', tenv, newlevel, breakpoint, body) in
              (match result with
                | Some(typ, pos) ->
                  (match S.look(tenv, typ) with
                    | Some(res_ty) ->
...
and transTy(tenv, ty) : T.ty = match ty with
  ...
  | A.RecordTy fields ->
    let field_tys = List.map (fun {A.name; typ; pos; _} ->
      match S.look(tenv, typ) with
        | Some(ty) -> (name, ty)
        | None -> error pos ("unknown type. " ^ S.name typ); (name, T.NIL)) fields in
